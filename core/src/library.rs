...
use crate::avm2::Class as Avm2Class;
...
/// A library character an AVM2 class instantiates, as defined by `SymbolClass`.
#[derive(Collect)]
#[collect(no_drop)]
struct ClassSymbol<'gc> {
    /// Weak, so a dead entry self-prunes once the movie is collected.
    movie: GcWeak<'gc, SwfMovie>,
    #[collect(require_static)]
    character_id: CharacterId,
}

/// Mappings between AVM2 class objects and the library characters they
/// instantiate, as defined by `SymbolClass`.
///
/// Kept here in `core` (keyed by class) so the AVM2 layer holds no reference to
/// `core` types. gc-arena-native equivalent of the `WeakValueHashMap` master
/// used (`weak-table` can't be used: `GcWeak::upgrade` needs a `Mutation`, which
/// `WeakElement::view` does not provide).
#[derive(Collect, Default)]
#[collect(no_drop)]
pub struct Avm2ClassRegistry<'gc> {
    /// Keyed by class pointer (so a registered class can still be collected);
    /// the movie is weak so dead entries self-prune.
    class_map: FnvHashMap<usize, ClassSymbol<'gc>>,
}

impl<'gc> Avm2ClassRegistry<'gc> {
    /// The (movie, character) an AVM2 class instantiates, or `None`.
    pub fn class_symbol(
        &self,
        class: Avm2Class<'gc>,
        mc: &Mutation<'gc>,
    ) -> Option<(Gc<'gc, SwfMovie>, CharacterId)> {
        let entry = self.class_map.get(&(class.as_ptr() as usize))?;
        let movie = entry.movie.upgrade(mc)?;
        Some((movie, entry.character_id))
    }

    /// Associate an AVM2 class with a library character. The first registration
    /// whose movie is still alive wins (matches Flash); later registrations,
    /// including from a different movie, are ignored.
    pub fn set_class_symbol(
        ...
        // Drop entries whose movie has been collected (self-cleaning weak map).
        self.class_map.retain(|_, e| e.movie.upgrade(mc).is_some());

        if let Some(old) = self.class_map.get(&(class.as_ptr() as usize)) {
            if old.movie.upgrade(mc).is_some_and(|m| Gc::ptr_eq(m, movie))
                && old.character_id != character_id
            {
                tracing::warn!(
...
pub struct Library<'gc> {
    ...
    /// Self-pruning weak map from a movie to its `MovieLibrary` (which is owned
    /// by a display object), so a library can be found by movie without walking
    /// the display tree. Weak values; dead entries drop on insert.
    movie_libraries: FnvHashMap<usize, GcWeak<'gc, RefLock<MovieLibrary<'gc>>>>,

    /// AVM2 class -> library character associations from `SymbolClass`.
...
impl<'gc> Library<'gc> {
    pub fn empty() -> Self {
        Self {
            ...
            movie_libraries: Default::default(),
    ...
    /// Register a movie's library so it can be found by [`Self::library_for_movie`].
    /// Dead entries are pruned here, keeping the map bounded by live movies.
    pub fn register_movie_library(
        &mut self,
        movie: Gc<'gc, SwfMovie>,
        library: Gc<'gc, RefLock<MovieLibrary<'gc>>>,
        mc: &Mutation<'gc>,
        ...
        self.movie_libraries
            .retain(|_, weak| weak.upgrade(mc).is_some());
        self.movie_libraries
            .insert(Gc::as_ptr(movie) as usize, Gc::downgrade(library));
    }

    /// Find a movie's `MovieLibrary` by pointer, or `None` if it was never
    /// registered or its library has been collected.
