...
impl Color {
    ...
    pub fn blend_over(&self, source: &Self) -> Self {
        ...
        #[inline(always)]
        fn scale(channel: u8, factor: u16) -> u8 {
            (channel as u16 * factor / 255) as u8
        }

        let r = source.red() + scale(self.red(), inv_sa);
        let g = source.green() + scale(self.green(), inv_sa);
        let b = source.blue() + scale(self.blue(), inv_sa);
        let a = source.alpha() + scale(self.alpha(), inv_sa);
        Self::rgba(r, g, b, a)
    }
